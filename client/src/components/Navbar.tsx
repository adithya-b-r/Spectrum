  ...
  return (
    <>
      ...
      <nav className="fixed w-full h-16 top-0 z-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 select-none">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-full flex justify-between items-center relative">
          {showMobileSearch ? (
            <div className="md:hidden flex items-center w-full gap-2 py-2">
              <button
                type="button"
                onClick={() => {
                  setShowMobileSearch(false);
                ...
                className="w-9 h-9 flex items-center justify-center rounded-full text-gray-600 dark:text-slate-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-slate-800 transition cursor-pointer flex-shrink-0"
                aria-label="Back"
              >
                <i className="bx bx-arrow-back text-xl"></i>
              </button>
              <form onSubmit={handleSearchSubmit} className="flex-1 relative flex items-center">
                <i className="bx bx-search absolute left-3 text-base text-gray-400 pointer-events-none"></i>
                <input
                  ...
                  className="w-full text-xs font-medium text-gray-800 dark:text-slate-100 bg-gray-100/80 dark:bg-slate-800 pl-9 pr-8 py-2 rounded-full border border-gray-200 dark:border-slate-700 focus:border-indigo-500 focus:bg-white dark:focus:bg-slate-800 outline-none transition"
                />
                {searchQuery && (
                  <button
                    type="button"
                    ...
                    className="absolute right-2.5 text-gray-400 hover:text-gray-600 dark:hover:text-slate-200 p-0.5 cursor-pointer"
                  >
            ...
            <>
              ...
              <div className="flex items-center flex-shrink-0">
                <div onClick={() => navigate("/")} className="flex items-center gap-2 sm:gap-2.5 cursor-pointer flex-shrink-0">
                  <img src="/logo2.png" alt="Spectrum" className="h-7 w-7 sm:h-8 sm:w-8 object-contain rounded-lg flex-shrink-0" />
                  <span className="text-lg sm:text-xl font-extrabold tracking-tight text-gray-900 dark:text-white">Spectrum</span>
                </div>
              ...
              <div className="flex items-center gap-1.5 sm:gap-3 flex-shrink-0">
                ...
                <button
                  ...
                  aria-label="Search"
                  className="md:hidden w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center rounded-full hover:bg-gray-100 dark:hover:bg-slate-800 text-gray-600 dark:text-slate-300 hover:text-gray-900 dark:hover:text-white transition cursor-pointer flex-shrink-0"
                >
                  <i className="bx bx-search text-lg sm:text-xl"></i>
                </button>
                ...
                {isLoading ? (
                  ...
                  <>
                    <button
                      ...
                      className="w-8 h-8 sm:w-auto sm:h-9 sm:px-4 flex items-center justify-center gap-2 rounded-full border border-indigo-200 dark:border-indigo-900 text-indigo-600 dark:text-indigo-400 bg-indigo-50/40 dark:bg-indigo-950/40 hover:bg-indigo-50 dark:hover:bg-indigo-900/60 text-sm font-semibold transition duration-200 cursor-pointer flex-shrink-0"
                    >
                      <i className="bx bx-edit text-base sm:text-base"></i>
                    ...
                    <div
                      ...
                      className="w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center relative text-gray-600 dark:text-slate-300 hover:text-gray-900 dark:hover:text-white rounded-full hover:bg-gray-100 dark:hover:bg-slate-800 transition cursor-pointer flex-shrink-0"
                      title="Notifications"
                    >
                      <i className="bx bx-bell text-lg sm:text-xl"></i>
                      {unreadCount > 0 && (
                        <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-[16px] sm:min-w-[18px] sm:h-[18px] px-1 bg-rose-600 text-white text-[9px] sm:text-[10px] font-bold rounded-full ring-2 ring-white dark:ring-slate-900 flex items-center justify-center animate-in zoom-in-75 duration-150">
                    ...
                    <div ref={profileMenuRef} className="relative">
                      <div
                        onClick={toggleNav}
                        className="flex items-center gap-1 cursor-pointer p-0.5 sm:pl-1 sm:py-1 sm:pr-2 rounded-full hover:bg-gray-100/80 dark:hover:bg-slate-800 transition"
                      >
                        <img
                          ...
                          className="w-7 h-7 sm:w-8 sm:h-8 rounded-full object-cover ring-1 ring-gray-200 dark:ring-slate-700"
                        />
                        <i className={`bx bx-chevron-down text-gray-500 dark:text-slate-400 text-sm sm:text-base transition-transform duration-200 ${displayNav ? 'rotate-180' : ''}`}></i>
                      </div>

                      {/* Profile Dropdown Menu */}
                      {displayNav && (
                        <div className="absolute flex flex-col top-full right-0 mt-2 bg-white dark:bg-slate-900 w-52 sm:w-56 rounded-2xl shadow-xl border border-gray-100 dark:border-slate-800 py-2 z-40 animate-in fade-in slide-in-from-top-2 duration-200">
                          <div
                            onClick={() => {
                              setdisplayNav(false);
                              navigate("/profile");
                  ...
                  <>
                    <button
                      ...
                      className="text-xs sm:text-sm font-semibold text-gray-700 dark:text-slate-200 hover:text-gray-900 dark:hover:text-white px-2 sm:px-3 py-1.5 whitespace-nowrap cursor-pointer transition flex-shrink-0"
                    >
                      Sign In
                    </button>
                    <button
                      onClick={() => setToggleRegister(true)}
                      className="text-xs sm:text-sm font-semibold bg-gray-900 hover:bg-black text-white dark:bg-white dark:hover:bg-slate-100 dark:text-slate-900 px-3 sm:px-4 py-1.5 rounded-full transition duration-200 shadow-sm cursor-pointer whitespace-nowrap flex-shrink-0"
                    >
                      Get Started
                    </button>
                  </>
        ...
        {showMobileSearch && showSearchDropdown && searchQuery.trim().length > 0 && (
          <div className="md:hidden fixed top-16 left-0 right-0 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800 shadow-2xl max-h-[calc(100vh-4rem)] overflow-y-auto z-50 animate-in fade-in slide-in-from-top-2 duration-150 p-2 space-y-3">
            {isSearching ? (
              <div className="p-6 text-center text-gray-400 flex items-center justify-center gap-2 text-xs font-medium">
                <i className="bx bx-loader-alt animate-spin text-base text-indigo-600"></i>
                <span>Searching Spectrum...</span>
                    ...
                    <div className="space-y-1">
                      {searchResults.users.slice(0, 3).map((u) => (
                        <div
                          key={u._id}
                          onClick={() => handleSelectAuthor(u.username, u._id)}
                          className="flex items-center gap-3 px-2.5 py-2 rounded-xl hover:bg-gray-50 dark:hover:bg-slate-800/80 transition cursor-pointer"
                        >
                          <img
                            src={u.profilePic || "/avatar.jpg"}
                            alt={u.fullName}
